...
public class drawPanel extends javax.swing.JPanel {
    ...
    protected List<Polygon> polyOut2;
    ...
    private static Path2D[] drawPoly(Polygon p){
        Path2D path [] = new Path2D[2];
        // in hrany
        path[0] = new Path2D.Double();
        // out hrany
        path[1] = new Path2D.Double();
        
        path[0].moveTo(p.edges.get(0).start.getX(), p.edges.get(0).start.getY());
        for (Edge e : p.edges){
            if (e.side == null || e.side == Algorithms.PositionEnum.OUTSIDE){
                path[1].moveTo(e.start.getX(), e.start.getY());
                path[1].lineTo(e.end.getX(), e.end.getY());
            }
            else { // inside
                path[0].moveTo(e.start.getX(), e.start.getY());
                path[0].lineTo(e.end.getX(), e.end.getY());
            }
